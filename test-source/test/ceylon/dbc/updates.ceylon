...
test void batchUpdateTest() {
    ...
    value result = sql.Update("UPDATE test1 SET name = ? WHERE count = ?").executeBatch(3, {for(i in 1..10) {"name``i*i``", i}});
    assertEquals(result.size, 10);
    assertEquals(result[0], 1);
    assertEquals(result[1], 1);
    assertEquals(result[2], 1);
    
    value rows = sql.Select("SELECT * FROM test1 ORDER BY count").execute();
