"""This module enables updates to multiple databases within a single transaction. It is integrated
   ...
   - the configuration approach is to define the datasources via a properties file whose
     ...
     [[http://java.sun.com/dtd/properties.dtd]] and an example is provided in the ceylon examples
     directory;
   ...
   - the programatic approach is via calls on the JNDI server to specify details of the
     ...
     tm.getJndiServer().registerDSUrl("h2", "org.h2.Driver", "jdbc:h2:tmp/ceylondb", "sa", "sa");

     registerDriverSpec takes the java class name of the JDBC driver, the module name and version of the
     module that provides the class and the java class name of the XA datasource.

     registerDSUrl (or registerDSName) provide details of how to connect to the datasource.
   ...
   Note that you must obtain the datasource via this lookup mechanism and not instantiate them directly.
