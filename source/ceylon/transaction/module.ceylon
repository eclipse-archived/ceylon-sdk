...
"""This module enables updates to multiple databases within 
   ...
   to obtain XA connections then the TransactionManager will
   ...
   After obtaining an XA connection you may use it just like any other
   ...
   [[dedicated process|ceylon.transaction.transactionRecoveryManager::start]].

   This recovery process needs to know which datasources
   any in doubt transactions were using prior to a failure so
   you will need to explicitly register them:
