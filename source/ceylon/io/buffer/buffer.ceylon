...
//TODO see (ByteBuffer, newByteBuffer, newByteBufferWithData,
...
shared abstract class Buffer<T>() satisfies Iterable<T> {
    ...
    "Returns the number of objects that can be read/written from the current
     [[position]] until we reach the [[limit]]."
    shared Integer available {
        return limit - position;
    }
    
    "Reads an object from this buffer at the current [[position]].
     Increases the [[position]] by `1`."
    shared formal T get();

    "Writes an object to this buffer at the current [[position]].
     Increases the [[position]] by `1`."
    ...
    "Returns an [[Iterator]] object to read from the current [[position]] until the
