...
shared interface TransactionRecoveryManager {
    ...
    shared formal void start(
            "The directory name of the location of the
             transaction logs. If null then the directory from
    ...
    "Recovery scans run periodically (the default is every 2 minutes).
     You may manually force an immediate recovery scan.
     A recovery scan attempts recovery on each entry in the transaction
     logs managed by this recovery manager instance"
    ...
    "Return a list of XA capable JDBC drivers that are known to work
     ...
     recommend exhaustive testing. Drivers that are known to work correctly include:
