"This module enables JVM thread concurrency, such that the most common use
 ...
 The primary class is [[Future]], which extends the multi-platform
 `Promise` from `ceylon.promise`. Typically [[Future]]s are created by a call
 to [[submit]], which will start the given callable running in a thread pool
 before returning the [[Future]]. You can then call [[Future.result]] to block
