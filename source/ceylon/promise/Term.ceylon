...
shared interface Term<out Element, out T>
	...
	"Combine the current term with a provided promise and return a new term object that
        - fulfills when both the current term and the other promise are fulfilled
        - rejects when the current term or the other promise is rejected
    
    The [[Term]] promise will be
        - fulfilled with a tuple of values of the original promise. It is important to notice that
    ...
    The returned [[Term]] object allows for promise chaining as a fluent API:
        ...
        Promise<Integer> p2 = ...
        Promise<Boolean> p3 = ...
        p1.and(p2, p3).compose((Boolean b, Integer i, String s) => doSomething(b, i, s));"
