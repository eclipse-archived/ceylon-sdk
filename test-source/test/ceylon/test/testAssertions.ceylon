...
void runTestAssertions() {
    runTests(
        ...
        `testAssertFalse`,
        `testAssertEquals`,
        `testAssertEqualsCompare`,
        `testAssertEqualsException`,
        `testAssertNotEquals`,
        `testAssertThatException`
    );
}
...
void testAssertThatException() {
    assertThatException(NegativeNumberException()).
        ...
        hasMessage((String m)=>m.startsWith("Negative")).
        hasNoCause();

    try {
        assertThatException(NegativeNumberException()).hasType(`OverflowException`);
        assert(false);
    }
    ...
    try {
        assertThatException(NegativeNumberException()).hasType((ClassModel<Exception, Nothing> t)=>false);
        assert(false);
    }
    catch(AssertionException e) {
    ...
    try {
        assertThatException(NegativeNumberException()).hasMessage("wops!");
        assert(false);
    }
    catch(AssertionException e) {
        assert(e.message == "assertion failed: expected exception with message wops!, but has Negative number");
    }

    try {
        assertThatException(NegativeNumberException()).hasMessage((String m)=>m.contains("wops!"));
        assert(false);
    }
    catch(AssertionException e) {
        assert(e.message == "assertion failed: expected different exception message than Negative number");
    }

    try {
        assertThatException(Exception("", NegativeNumberException())).hasNoCause();
        assert(false);
    }
    catch(AssertionException e) {
        assert(e.message == "assertion failed: expected exception without cause, but has ceylon.language.NegativeNumberException \"Negative number\"");
    }
