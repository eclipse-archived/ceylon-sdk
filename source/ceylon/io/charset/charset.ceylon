...
"Represents a character set, which allows you to convert characters to bytes
 and back.
...
shared interface Charset {
    ...
    "This character set's name. In general this is the _preferred MIME name_
    ...
    "Returns a new [[Encoder]] which allows you to encode characters
     into byte buffers."
    shared formal Encoder newEncoder();

    "The minimum number of bytes taken when encoding a character into bytes."    
    shared formal Integer minimumBytesPerCharacter;

    "The maximum number of bytes taken when encoding a character into bytes.
     Defaults to [[minimumBytesPerCharacter]]."    
    shared default Integer maximumBytesPerCharacter {
        return minimumBytesPerCharacter;
    }

    "The average number of bytes taken when encoding a character into bytes.
    ...
    "Encodes the given [[string]] into a newly-created [[ByteBuffer]]. This is a convenience method."
    ...
    "Decodes the given [[ByteBuffer]] into a newly-created [[String]]. This is a convenience method."
...
"Allows you to encode a sequence of characters into a sequence of bytes."
by("Stéphane Épardaud")
shared interface Encoder {
    ...
    "Returns true if there are no bytes pending to be output. Returns false
    ...
    "Encodes the given [[input]] character buffer into the given [[output]]
...
"Allows you to decode a sequence of bytes into a sequence of characters."
by("Stéphane Épardaud")
shared interface Decoder {
    ...
    "Decodes the given byte [[buffer]] into an underlying character buffer.
     Attempts to decode as many bytes as are available."
    shared formal void decode(ByteBuffer buffer);

    "Returns a [[String]] consisting of all the decoded characters so far.
