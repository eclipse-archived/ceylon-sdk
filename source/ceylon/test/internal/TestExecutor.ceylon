...
class FunctionTestExecutor(FunctionDeclaration funcDecl) satisfies TestExecutor {
    ...
    void handleTestExecution(TestListener notifier, Anything() handler)() {
        value startTime = process.milliseconds;
        function elapsedTime() => process.milliseconds - startTime; 
        try {
            notifier.testStart(description);
            handler();
    ...
    void invokeFunction(FunctionDeclaration fd) {
        if( fd.toplevel ) {
            Function<Anything,[]> f = fd.apply<Anything, []>();
            f();        
        }
        else {
            value instance = getInstance();
            // TODO temporary workaround, until metamodel will not support "unsafe invoke"
            Util().invoke(instance, fd.name);
        }
    }
