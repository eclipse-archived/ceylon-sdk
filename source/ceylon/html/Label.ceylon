 ...
 ********************************************************************************/
"
 The __&lt;label&gt;__ element represents a caption for an item in a user interface. 
 It can be associated with a control either by placing the control element inside the 
 label element, or by using the for attribute. Such a control is called the labeled 
 control of the label element. One input can be associated with multiple labels.
 ...
 [Official W3C reference](https://www.w3.org/TR/html5/sections.html#the-lebel-elements).
"
...
shared class Label(
    ...
    "Attribute represents the ID of a labelable form-related element in the same document as the label element."
    Attribute<String> forElement = null,
    ...
    shared actual {Content<PhrasingCategory>*} children = [])
        extends Element("label", id, clazz, accessKey, contentEditable, contextMenu, dir, draggable, dropZone, hidden, lang, spellcheck, style, tabIndex, title, translate, 
            [attributeEntry("for", forElement), attributeEntry("form", form), *attributes], children)
